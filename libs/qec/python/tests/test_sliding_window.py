...
def run_decoder(filename, num_shots, run_as_batched, run_as_sliding_window):
    """
    ...
    if run_as_sliding_window:
        sliding_window_args = {
            ...
            'inner_decoder_params': nv_dec_args
        }

    print(f'H shape: {H.shape}')
    print(f'obs_mat shape: {obs_mat.shape}')
    try:
        if run_as_sliding_window:
            ix = qec.get_sorted_pcm_column_indices(H, sliding_window_args['num_syndromes_per_round'])
            H = qec.reorder_pcm_columns(H, ix)
            obs_mat = obs_mat[ix, :]
            sliding_window_args['error_rate_vec'] = error_rate_vec[ix]
            nv_dec_gpu_and_cpu = qec.get_decoder("sliding_window", H,
                                                 **sliding_window_args)
        else:
