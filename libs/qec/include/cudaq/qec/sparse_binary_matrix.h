...
public:
  ...
  static sparse_binary_matrix from_nested_csc(
  ...
  static sparse_binary_matrix from_nested_csr(
      index_type num_rows, index_type num_cols,
      const std::vector<std::vector<index_type>> &nested);
  ...
  sparse_binary_matrix(const cudaqx::tensor<std::uint8_t> &dense,
  ...
  index_type num_nnz() const {
    return indices_.empty() ? 0
                            : static_cast<index_type>(indices_.size());
  }
